...
/datum/nano_module/deck_management
	...
	var/default_access = list(access_command, access_command)  //The format is (needs one of list(these access constants or lists of access constants))
...
/datum/nano_module/deck_management/proc/get_shuttle_access(mob/user, datum/shuttle/shuttle)
	return shuttle.logging_access ? (check_access(user, shuttle.logging_access) || check_access(user, access_command)) : 0
